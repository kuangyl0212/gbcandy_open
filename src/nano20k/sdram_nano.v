...
// Based on SNESTang by nand2mario
// 
// This supports 3 parallel access streams (ROM/WRAM/BSRAM/RiscV, ARAM, Frame Buffer),
// independent from each other. SNES ROM/WRAM uses bank 0 (8MB, largest game is 6MB, 
// BSRAM max 1MB). RV and BSRAM uses bank 1. ARAM uses bank 2. Frame buffer uses bank 3. 
...
(
    ...
    // Frame buffer write port (PPU → SDRAM, Bank 3)
    // 2-pixel packed: hi16=even pixel, lo16=odd pixel
    input  [15:0] fb_wr_addr,
    input  [15:0] fb_wr_din,
    input  [15:0] fb_wr_din_hi,
    input        fb_wr_req,
    output reg   fb_wr_ack,
    input        fb_wr_we,

    // Frame buffer read port (SDRAM → line buffer, Bank 3)
    input  [15:0] fb_rd_addr,
    output reg [31:0] fb_rd_dout,
    output reg [31:0] fb_rd_dq_raw,
    output reg        fb_rd_dq_valid,
    input        fb_rd_req,
    output reg   fb_rd_ack,
...
localparam PORT_FB_RD = 2'd1;
localparam PORT_FB_WR = 2'd2;

reg  [1:0] port[3];
reg  [1:0] next_port[3];
reg [22:0] next_addr[3];        // 2-bit bank #, then 2MB byte address in bank
reg [15:0] next_din[3];
reg [15:0] next_din_lo[3];
reg  [1:0] next_ds[3];
reg  [2:0] next_we;
reg  [2:0] next_oe;
...
always @(*) begin
	...
	next_din_lo[2] = 0;
	next_ds[2] = 0;
    // Read has priority over write (prevent HDMI tearing)
    if (fb_rd_req ^ fb_rd_ack) begin
        next_port[2] = PORT_FB_RD;
        next_addr[2] = {5'b11_111, fb_rd_addr, 2'b00};
        next_we[2] = 1'b0;
        next_oe[2] = 1'b1;
        next_ds[2] = 2'b11;
    end else if (fb_wr_req ^ fb_wr_ack) begin
        next_port[2] = PORT_FB_WR;
        next_addr[2] = {5'b11_111, fb_wr_addr, 2'b00};
        next_we[2] = fb_wr_we;
        next_oe[2] = ~fb_wr_we;
        next_din[2] = fb_wr_din_hi;
        next_din_lo[2] = fb_wr_din;
        next_ds[2] = 2'b11;
    end
end
...
always @(posedge clk) begin
    if (~resetn) begin
        ...
        if (normal) begin
            ...
            // Refresh
            if (cycle[2] && need_refresh && !(fb_wr_req^fb_wr_ack) && !(fb_rd_req^fb_rd_ack)
                && !we_latch[0] && !oe_latch[0] && !we_latch[1] && !oe_latch[1]) begin
                // REFRESH if there's no ongoing CPU/ARAM nor upcoming frame buffer requests
                refresh <= 1'b1;
                refresh_cnt <= 0;
                cmd <= CMD_AutoRefresh;
            ...
            if(cycle[7] && (oe_latch[2] || we_latch[2])) begin
                ...
                if (we_latch[2]) begin
                    dq_oen <= 1'b0;
                    dq_out <= {din_latch[2], din_latch_lo[2]};
                    SDRAM_DQM <= 4'b0000;
                end else
                    SDRAM_DQM <= 4'b0;
            ...
            // Frame buffer write data: keep driving DQ for CAS=2 (data needed at cycle[1])
            if(cycle[0] && we_latch[2] && port[2] == PORT_FB_WR) begin
                dq_oen <= 1'b0;  // 保持驱动，确保数据在 CAS Write Latency 周期可用
                dq_out <= {din_latch[2], din_latch_lo[2]};
            end
            if(cycle[7]) begin
                case (port[2])
                    PORT_FB_RD: fb_rd_ack <= fb_rd_req;
                    PORT_FB_WR: fb_wr_ack <= fb_wr_req;
                    default: ;
                endcase
            end
            ...
            // CAS=2: Read CAS at cycle[7], data sampled at cycle[2] (3 cycles, same as channel 0)
            if (cycle[2] && oe_latch[2]) begin
                if (port[2] == PORT_FB_RD) begin
                    fb_rd_dout <= dq_in;
                    fb_rd_dq_raw <= dq_in;
                    fb_rd_dq_valid <= 1'b1;
                end
