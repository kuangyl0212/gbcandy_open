...
//
// Based on VerilogBoy sound_length_ctr.v
...
module apu_length_ctr #(
    ...
    output reg enable = 0,
    output reg [WIDTH-1:0] length_left_debug = {WIDTH{1'b0}},
    // Sticky diagnostics: capture historical behavior during test execution
    output reg [WIDTH-1:0] dbg_max_len_seen = {WIDTH{1'b0}},
    output reg        dbg_ever_started = 1'b0,
    output reg        dbg_ever_disabled = 1'b0,
    output reg [WIDTH-1:0] dbg_disable_len = {WIDTH{1'b0}},
    output reg [7:0]  dbg_start_count = 8'h0,
    output reg [WIDTH-1:0] dbg_trigger_len = {WIDTH{1'b0}}
);
    ...
    always @(posedge clk) begin
        if (rst) begin
            length_left_debug <= {WIDTH{1'b0}};
        end else begin
            length_left_debug <= length_left;

            if (dbg_max_len_seen < length_left)
                dbg_max_len_seen <= length_left;

            if (start_rise) begin
                dbg_ever_started <= 1'b1;
                dbg_start_count <= dbg_start_count + 8'h1;
                dbg_trigger_len <= (length == 0) ? {WIDTH{1'b1}} : length;
            end

            if (clk_length_ctr_rise && single && (length_left == {WIDTH{1'b1}}) && !enable) begin
                dbg_ever_disabled <= 1'b1;
                dbg_disable_len <= length_left;
            end
        end
    end
