...
//
// Based on VerilogBoy cpu.v by Wenting Zhang
//
// Changes from original:
//   1. Added `enable` input: when low, the entire CPU state machine freezes.
//      This replaces the external 4.19MHz clock with mclk (21.6MHz) + CE gating.
//      When `enable` is asserted every 5th mclk cycle, the CPU sees an effective
//      4.32MHz clock — close enough to the Game Boy's 4.19MHz.
//   2. Added d_bus_op[1:0] and d_m_cycle[2:0] debug outputs for trace capture.
//   3. CPU starts at $0000 (no boot ROM: runs cartridge ROM directly).
//
// All original logic is preserved exactly. Do NOT add additional CE gating here.
// The `enable` signal is equivalent to running clk at ~4.19MHz.
////////////////////////////////////////////////////////////////////////////////

module cpu(
    input  clk,
    input  rst,
    input  enable,             // CPU enable: high = advance, low = freeze
    output reg phi,
    output wire [1:0] ct,
    output reg [15:0] a,
    output reg [7:0] dout,
    ...
    output wire stop,
    output wire halt,
    input            cgb_speed_switch,
    output wire speed_switch,

    output wire [7:0]  d_opcode,
    output wire [15:0] d_pc,
    output wire [15:0] d_last_pc,
    output wire [1:0]  d_bus_op,   // 01=instruction fetch, 10=write, 11=data read
    output wire [2:0]  d_m_cycle,   // Machine cycle counter
    output wire        d_int_master_en,  // IME (Interrupt Master Enable) for debug
    output wire [15:0] d_sp              // Stack pointer for debug
    );
    ...
    control control(
        .clk(clk),
        .rst(rst),
        .enable(enable),            // CPU enable propagates to control latch
        .ex_next_state(3'b0),       // unused inside control, tie to 0
        .opcode_early(opcode),
        .cb(cb),
        .imm(imm_low),
        ...
        .wake(wake),
        .fault(fault),
        .cgb_speed_switch(cgb_speed_switch),
    ...
    assign rf_we = enable && (alu_dst == 2'b10) && (!temp_redir);
    ...
    // acc_we must be gated by enable to prevent ACC corruption during SDRAM wait
    assign acc_we = enable && ((alu_dst == 2'b00) || ((db_src == 2'b00) && (bus_op == 2'b11)));
    ...
    //reg [15:0] imm_reg; declared before
    ...
    // EX - FSM / Multi-M-cycle Instruction Handling
    reg  [2:0] ex_state;
    ...
    assign d_bus_op   = bus_op;
    assign d_m_cycle  = m_cycle;
    assign d_int_master_en = int_master_en;
    assign d_sp            = rf_sp;
