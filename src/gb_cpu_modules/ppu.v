...
module ppu(
    input clk,
    input rst,
    input enable,        // CE gate: high = advance, low = freeze (cpu_ce_combined)
    ...
    output wire vram_access_ext_out,
    input wire [7:0]  vram_din,
    input wire        vram_rd,
    input wire        vram_wr,
    output wire [12:0] vram_addr_int_out,
    output wire vram_rd_int_out,
    output wire [7:0] vram_data_out_int,
    input wire [7:0]  vram_data_in_int,
    input wire [7:0]  vram_data_in_int_bank1,
    input wire        cgb_mode,
    input wire        obj_prio_dmg_mode,
    input wire [15:0] oam_a,
    input wire [7:0]  oam_din,
    input wire        oam_wr,
    output wire oam_access_ext_out,
    output wire [6:0] oam_addr_int_out,
    output wire oam_rd_int_out,
    input wire [15:0] oam_data_in_int,
    output wire [7:0] oam_data_out_int,
    ...
    output [7:0] d_line,    // LY register (v_count)
    output [8:0] d_h_count, // horizontal counter
    output [1:0] d_mode,    // Current PPU mode (for OAM bug detection)
    output [7:0] d_reg_bgp,
    output [7:0] d_reg_obp0,
    output [7:0] d_reg_obp1,
    output [7:0] d_reg_lyc,
    output reg [2:0] cgb_palette_out,
    output reg       cgb_bg_priority_out,
    output reg [1:0] color_idx_out,
    output reg [1:0] palette_id_out
    );
    ...
    // OAM - External interface (using Gowin DP BSRAM primitive via gb_oam_dp)
    // CPU accesses OAM through external gb_oam_dp module during H/V-blank
    
    assign oam_addr_int_out = oam_rd_addr_int[7:1];
    assign oam_rd_int_out = oam_access_int;
    assign oam_data_out_int = 8'h00;
    assign oam_access_ext_out = oam_access_ext;

    // OAM data from external DP BSRAM
    wire [15:0] oam_data_out = oam_data_in_int;
    
    // CPU read: byte select from 16-bit data
    assign oam_dout = (oam_access_ext) ? ((oam_a[0]) ? oam_data_out[15:8] : oam_data_out[7:0]) : 8'hFF;
    
    // OAM internal read address
    reg [7:0] oam_rd_addr_int;

    // VRAM - External interface
    // PPU outputs internal VRAM access signals for external VRAM instance
    // CPU accesses VRAM through external interface during H/V-blank
    
    assign vram_addr_int_out = vram_addr_int;
    assign vram_rd_int_out = vram_access_int;
    assign vram_data_out_int = 8'h00;  // PPU only reads, never writes
    assign vram_access_ext_out = vram_access_ext;  // CPU can access VRAM
    
    // VRAM data from external RAM
    wire [7:0] vram_data_out = vram_data_in_int;
    wire [7:0] vram_data_out_bank1 = vram_data_in_int_bank1;
    ...
    reg [47:0] pf_cgb_pal;  // CGB palette FIFO: 3 bits/pixel × 16 pixels
    reg [15:0] pf_cgb_prio; // CGB BG priority FIFO: 1 bit/pixel × 16 pixels
    reg [63:0] pf_cgb_obj_idx; // CGB OBJ OAM index FIFO: 4 bits/pixel × 16 pixels
    ...
    wire [2:0] pf_output_cgb_palette = pf_cgb_pal[47:45];
    wire       pf_output_cgb_priority = pf_cgb_prio[15];
    ...
    begin
        if (rst) begin
            ...
            if (!reg_lcd_en) begin
                // DMG-compatible LCD enable sync point:
                // When LCD is off, the PPU halts.
                // When turning LCD on, LY should increment after ~110 CPU cycles.
                // Our PPU_H_TOTAL is 456. So starting at 456 - 109 = 347.
                // Let's set h_count to 347 to match the blargg 1-lcd_sync delay.
                h_count <= 9'd347;
                v_count <= 8'd0;
                hs <= 1'b0;
                vs <= 1'b0;
    ...
    reg [63:0] sww_clr_data;
    reg [47:0] sww_clr_cgb_pal;
    reg [15:0] sww_clr_cgb_prio;
    reg [63:0] sww_clr_cgb_obj_idx;
    always @(pf_data, pf_cgb_pal, pf_cgb_prio, pf_cgb_obj_idx) begin
        for (i = 0; i < 16; i = i + 1) begin
            if (pf_data[i*4+1] == PPU_PAL_BG[1] && pf_data[i*4] == PPU_PAL_BG[0]) begin
                sww_clr_data[i*4+3] = 1'b0;
                sww_clr_data[i*4+2] = 1'b0;
                sww_clr_data[i*4+1] = 1'b0;
                sww_clr_data[i*4]   = 1'b0;
                sww_clr_cgb_pal[i*3+2] = 1'b0;
                sww_clr_cgb_pal[i*3+1] = 1'b0;
                sww_clr_cgb_pal[i*3]   = 1'b0;
                sww_clr_cgb_prio[i] = 1'b0;
                sww_clr_cgb_obj_idx[i*4+3] = 1'b0;
                sww_clr_cgb_obj_idx[i*4+2] = 1'b0;
                sww_clr_cgb_obj_idx[i*4+1] = 1'b0;
                sww_clr_cgb_obj_idx[i*4]   = 1'b0;
            end
            else begin
                sww_clr_data[i*4+3] = pf_data[i*4+3];
                sww_clr_data[i*4+2] = pf_data[i*4+2];
                sww_clr_data[i*4+1] = pf_data[i*4+1];
                sww_clr_data[i*4]   = pf_data[i*4];
                sww_clr_cgb_pal[i*3+2] = pf_cgb_pal[i*3+2];
                sww_clr_cgb_pal[i*3+1] = pf_cgb_pal[i*3+1];
                sww_clr_cgb_pal[i*3]   = pf_cgb_pal[i*3];
                sww_clr_cgb_prio[i] = pf_cgb_prio[i];
                sww_clr_cgb_obj_idx[i*4+3] = pf_cgb_obj_idx[i*4+3];
                sww_clr_cgb_obj_idx[i*4+2] = pf_cgb_obj_idx[i*4+2];
                sww_clr_cgb_obj_idx[i*4+1] = pf_cgb_obj_idx[i*4+1];
                sww_clr_cgb_obj_idx[i*4]   = pf_cgb_obj_idx[i*4];
            end
        end
    end
    ...
    reg [7:0] win_line_counter = 8'd0;

    wire [2:0] line_to_tile_v_offset_bg = v_pix_in_map[2:0];
    wire [4:0] line_in_tile_v_bg = v_pix_in_map[7:3];
    wire [2:0] line_to_tile_v_offset_win = win_line_counter[2:0];
    wire [4:0] line_in_tile_v_win = win_line_counter[7:3];
    wire [2:0] line_to_tile_v_offset = (render_window_or_bg) ? (line_to_tile_v_offset_win) : (line_to_tile_v_offset_bg);
    wire [2:0] line_to_tile_v_offset_cgb = (cgb_mode && current_bg_cgb_y_flip) ? ~line_to_tile_v_offset : line_to_tile_v_offset;
    ...
    wire [12:0] current_tile_address_0 = (bg_window_tile_addr) + current_tile_id_adj * 16 + (line_to_tile_v_offset_cgb * 2);
    wire [12:0] current_tile_address_1 = (current_tile_address_0) | 13'h0001;
    reg [7:0] current_tile_data_0;
    reg [7:0] current_tile_data_1;

    reg [7:0] current_tile_attr;
    wire [2:0] current_bg_cgb_palette = current_tile_attr[2:0];
    wire       current_bg_cgb_priority = current_tile_attr[7];
    wire       current_bg_cgb_vram_bank = current_tile_attr[3];
    wire       current_bg_cgb_x_flip = current_tile_attr[5];
    wire       current_bg_cgb_y_flip = current_tile_attr[6];

    wire [7:0] obj_vram_data = (cgb_mode && current_obj_cgb_vram_bank) ? vram_data_out_bank1 : vram_data_out;
    wire [7:0] bg_vram_data = (cgb_mode && current_bg_cgb_vram_bank) ? vram_data_out_bank1 : vram_data_out;

    // Data that will be pushed into pixel FIFO
    // Organized in pixels
    reg [31:0] current_fetch_result;
    always@(current_tile_data_1, current_tile_data_0, current_bg_cgb_x_flip, cgb_mode) begin
        for (i = 0; i < 8; i = i + 1) begin
            if (cgb_mode && current_bg_cgb_x_flip) begin
                current_fetch_result[i*4+3] = current_tile_data_1[7-i];
                current_fetch_result[i*4+2] = current_tile_data_0[7-i];
            end else begin
    ...
    wire [7:0] oam_search_y = oam_data_out[7:0];
    wire [7:0] oam_search_x = oam_data_out[15:8];
    // OAM search data from external RAM
    ...
    wire [2:0] current_obj_cgb_palette = current_obj_flags[2:0];
    wire       current_obj_cgb_vram_bank = current_obj_flags[3];
    /* verilator lint_off WIDTH */
    ...
    reg [23:0] merge_cgb_pal;
    reg [7:0]  merge_cgb_prio;
    reg [31:0] merge_cgb_obj_idx;
    always@(*) begin
        for (i = 0; i < 8; i = i + 1) begin
            if (
                    ...
                    (
                        ...
                        ||
                        (cgb_mode && !obj_prio_dmg_mode &&
                         !((pf_data[32+i*4+1] == PPU_PAL_BG[1])&&(pf_data[32+i*4+0] == PPU_PAL_BG[0])) &&
                         (obj_trigger_id[0] != pf_cgb_obj_idx[32+i*4+0] ||
                          obj_trigger_id[1] != pf_cgb_obj_idx[32+i*4+1] ||
                          obj_trigger_id[2] != pf_cgb_obj_idx[32+i*4+2] ||
                          obj_trigger_id[3] != pf_cgb_obj_idx[32+i*4+3]) &&
                         (obj_trigger_id < {pf_cgb_obj_idx[32+i*4+3], pf_cgb_obj_idx[32+i*4+2], pf_cgb_obj_idx[32+i*4+1], pf_cgb_obj_idx[32+i*4+0]}))
                    )&&
                    !(
                        cgb_mode &&
                        reg_bg_disp &&
            ...
            begin
                ...
                merge_cgb_pal[i*3+2] = current_obj_cgb_palette[2];
                merge_cgb_pal[i*3+1] = current_obj_cgb_palette[1];
                merge_cgb_pal[i*3+0] = current_obj_cgb_palette[0];
                merge_cgb_prio[i] = current_obj_to_bg_priority;
                merge_cgb_obj_idx[i*4+3] = obj_trigger_id[3];
                merge_cgb_obj_idx[i*4+2] = obj_trigger_id[2];
                merge_cgb_obj_idx[i*4+1] = obj_trigger_id[1];
                merge_cgb_obj_idx[i*4+0] = obj_trigger_id[0];
            end
            else begin
                ...
                merge_cgb_pal[i*3+2] = pf_cgb_pal[24+i*3+2];
                merge_cgb_pal[i*3+1] = pf_cgb_pal[24+i*3+1];
                merge_cgb_pal[i*3+0] = pf_cgb_pal[24+i*3+0];
                merge_cgb_prio[i] = pf_cgb_prio[8+i];
                merge_cgb_obj_idx[i*4+3] = pf_cgb_obj_idx[32+i*4+3];
                merge_cgb_obj_idx[i*4+2] = pf_cgb_obj_idx[32+i*4+2];
                merge_cgb_obj_idx[i*4+1] = pf_cgb_obj_idx[32+i*4+1];
                merge_cgb_obj_idx[i*4+0] = pf_cgb_obj_idx[32+i*4+0];
            end
        end
    end
    ...
    // OAM search data from internal OAM registers
    ...
    begin
        ...
        case (r_state)
            ...
            S_BLANK: begin
                ...
                if (window_triggered)
                    win_line_counter <= win_line_counter + 1'b1;
                window_triggered <= 1'b0;
                if (is_in_v_blank && v_count == PPU_V_TOTAL - 1)
                    win_line_counter <= 8'd0;
            end
            S_OAMX: begin
            ...
            S_FTIDB: begin
                ...
                current_tile_attr <= cgb_mode ? vram_data_out_bank1 : 8'h00;
            end

            S_FRD0A: vram_addr_bg <= current_tile_address_0;
            S_FRD0B: current_tile_data_0 <= bg_vram_data;
            S_FRD1A: vram_addr_bg <= current_tile_address_1;
            S_FRD1B: begin
                current_tile_data_1 <= bg_vram_data;
            ...
            S_OFRD0B:
                if (current_obj_x_flip == 1'b1)
                    current_obj_tile_data_0[7:0] <= {
                        obj_vram_data[0], obj_vram_data[1], obj_vram_data[2], obj_vram_data[3],
                        obj_vram_data[4], obj_vram_data[5], obj_vram_data[6], obj_vram_data[7]
                    };
                else
                    current_obj_tile_data_0 <= obj_vram_data;
            ...
            S_OFRD1B:
                if (current_obj_x_flip == 1'b1)
                    ...
                    current_obj_tile_data_1 <= obj_vram_data;
    ...
    reg [23:0] half_merge_cgb_pal;
    reg [7:0]  half_merge_cgb_prio;
    reg [31:0] half_merge_cgb_obj_idx;
    always @(current_fetch_result, pf_data, pf_cgb_pal, pf_cgb_prio, pf_cgb_obj_idx, current_bg_cgb_palette, current_bg_cgb_priority) begin
        for (i = 0; i < 8; i = i + 1) begin
            if ((pf_data[32+i*4+1] == PPU_PAL_BG[1])&&(pf_data[32+i*4+0] == PPU_PAL_BG[0])) begin
                ...
                half_merge_cgb_pal[i*3+2] = current_bg_cgb_palette[2];
                half_merge_cgb_pal[i*3+1] = current_bg_cgb_palette[1];
                half_merge_cgb_pal[i*3+0] = current_bg_cgb_palette[0];
                half_merge_cgb_prio[i] = current_bg_cgb_priority;
                half_merge_cgb_obj_idx[i*4+3] = 1'b1;
                half_merge_cgb_obj_idx[i*4+2] = 1'b1;
                half_merge_cgb_obj_idx[i*4+1] = 1'b1;
                half_merge_cgb_obj_idx[i*4+0] = 1'b1;
            end
            else if (pf_cgb_prio[8+i] && !(current_fetch_result[i*4+3] == 1'b0 && current_fetch_result[i*4+2] == 1'b0)) begin
                half_merge_result[i*4+3] = current_fetch_result[i*4+3];
                half_merge_result[i*4+2] = current_fetch_result[i*4+2];
                half_merge_result[i*4+1] = current_fetch_result[i*4+1];
                half_merge_result[i*4+0] = current_fetch_result[i*4+0];
                ...
                half_merge_cgb_pal[i*3+2] = pf_cgb_pal[24+i*3+2];
                half_merge_cgb_pal[i*3+1] = pf_cgb_pal[24+i*3+1];
                half_merge_cgb_pal[i*3+0] = pf_cgb_pal[24+i*3+0];
                half_merge_cgb_prio[i] = pf_cgb_prio[8+i];
                half_merge_cgb_obj_idx[i*4+3] = pf_cgb_obj_idx[32+i*4+3];
                half_merge_cgb_obj_idx[i*4+2] = pf_cgb_obj_idx[32+i*4+2];
                half_merge_cgb_obj_idx[i*4+1] = pf_cgb_obj_idx[32+i*4+1];
                half_merge_cgb_obj_idx[i*4+0] = pf_cgb_obj_idx[32+i*4+0];
            end
        end
    end
            ...
            begin
                ...
                if (pf_empty == PF_EMPTY) begin
                    ...
                    // We need to be careful not to overwrite the sprites...
                        ...
                        pf_cgb_pal[47:24] <= half_merge_cgb_pal[23:0];
                        pf_cgb_prio[15:8] <= half_merge_cgb_prio[7:0];
                        pf_cgb_obj_idx[63:32] <= half_merge_cgb_obj_idx[31:0];
                    end
                end
                    ...
                    if (r_state == S_FTIDA) begin // reload and shift
                        if (pf_empty == PF_INITB) begin
                            ...
                            pf_cgb_pal[47:0] <= {15'b0, {8{current_bg_cgb_palette}}, 9'b0};
                            pf_cgb_prio[15:0] <= {5'b0, {8{current_bg_cgb_priority}}, 3'b0};
                            pf_cgb_obj_idx[63:0] <= {20'b0, {8{4'hF}}, 12'b0};
                        end
                        else begin // PF_FULL or PF_FIN
                            pf_data[63:0] <= {pf_data[59:32], current_fetch_result[31:0], 4'b0};
                            pf_cgb_pal[47:0] <= {pf_cgb_pal[44:24], {8{current_bg_cgb_palette}}, 3'b0};
                            pf_cgb_prio[15:0] <= {pf_cgb_prio[14:8], {8{current_bg_cgb_priority}}, 1'b0};
                            pf_cgb_obj_idx[63:0] <= {pf_cgb_obj_idx[59:32], {8{4'hF}}, 4'b0};
                        end
                    end
                    else begin // just shift
                        pf_data <= {pf_data[59:0], 4'b0};
                        pf_cgb_pal <= {pf_cgb_pal[44:0], 3'b0};
                        pf_cgb_prio <= {pf_cgb_prio[14:0], 1'b0};
                        pf_cgb_obj_idx <= {pf_cgb_obj_idx[59:0], 4'b0};
                    end
                    ...
                    if (h_drop != 3'd0) begin
                        ...
                        // Output valid pixels after first 8 null pixels
                        // (for objects with x < 8)
                        if (h_pix_output >= 8)
                            valid <= 1;
                        else
                            valid <= 0;
                        if (!cgb_mode && !reg_bg_disp && pf_output_palette_id == PPU_PAL_BG) begin
                            pixel <= reg_bgp[1:0];
                            color_idx_out <= 2'b00;
                        end else begin
                            pixel <= pf_output_pixel;
                            color_idx_out <= pf_output_pixel_id;
                        end
                        cgb_palette_out <= pf_output_cgb_palette;
                        cgb_bg_priority_out <= pf_output_cgb_priority;
                        palette_id_out <= pf_output_palette_id;
                ...
                pf_cgb_pal <= {merge_cgb_pal[23:0], pf_cgb_pal[23:0]};
                pf_cgb_prio <= {merge_cgb_prio[7:0], pf_cgb_prio[7:0]};
                pf_cgb_obj_idx <= {merge_cgb_obj_idx[31:0], pf_cgb_obj_idx[31:0]};
                valid <= 1'b0;
            end
            else if (r_state == S_SWW) begin
                pf_empty <= PF_EMPTY;
                pf_data <= sww_clr_data;
                pf_cgb_pal <= sww_clr_cgb_pal;
                pf_cgb_prio <= sww_clr_cgb_prio;
                pf_cgb_obj_idx <= sww_clr_cgb_obj_idx;
                valid <= 1'b0;
            end
            else begin
    ...
    begin
        case (r_state)
            S_IDLE: r_next_state = (reg_lcd_en) ? (S_BLANK) : (S_IDLE);
    ...
    begin
        if (rst) begin
            reg_lcdc <= 8'h91;  // No boot ROM: LCD on, BG display, 8000 mode (DMG boot ROM end state)
            reg_stat[7:3] <= 5'h00;
            reg_scy  <= 8'h00;
            reg_scx  <= 8'h00;
            reg_lyc  <= 8'h00;
    ...
    assign d_line = reg_ly;
    assign d_h_count = h_count;
    assign d_mode = reg_mode;
    assign d_reg_lyc = reg_lyc;
    assign d_reg_bgp = reg_bgp;
    assign d_reg_obp0 = reg_obp0;
    assign d_reg_obp1 = reg_obp1;
