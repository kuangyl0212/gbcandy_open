...
module control(
    ...
    output reg       int_ack,
    output reg       next,
    output           comb_next_out,
    output           comb_pc_jr_out,
    output reg       stop,
    output reg       halt,
    input            wake,
    output reg       fault,
    input            cgb_speed_switch,
    output reg       speed_switch
    );
    ...
    always @(posedge clk) begin
        if (rst)
            ime <= 1'b0;  // Boot ROM runs with IME=0; game code must execute EI
        else if (ime_clear)
            ime <= 1'b0;
        else if (ime_set)
            ime <= 1'b1;
    ...
    reg stop_last;
    reg fault_last;
    reg stop_speed_latch;
    always @(posedge clk) begin
        if (rst) begin
            halt_last <= 1'b0;
            stop_last <= 1'b0;
            fault_last <= 1'b0;
            stop_speed_latch <= 1'b0;
        end
        else begin
            halt_last <= halt;
            stop_last <= stop;
            fault_last <= fault;
            if (enable) begin
                if (opcode == 8'h10 && m_cycle == 0 && ct_state == 2'b11)
                    stop_speed_latch <= cgb_speed_switch;
                else if (m_cycle == 0 && ct_state == 2'b00)
                    stop_speed_latch <= 1'b0;
            end
        end
    end
    ...
    reg       comb_int_ack;
    reg       comb_next;
    assign comb_next_out = comb_next;
    assign comb_pc_jr_out = comb_pc_jr;
    reg       comb_stop;
    reg       comb_halt;
    reg       comb_fault;
        ...
        if (!comb_halt && !comb_stop && !comb_fault && !int_dispatch) begin
            ...
            // JR CC, imm8
                ...
                if (m_cycle == 0) begin
                    ...
                    if (((opcode == 8'h20) && (!f_z)) ||
                        ...
                        ((opcode == 8'h18)) ||
                        ((opcode == 8'h28) && (f_z)) ||
                        ((opcode == 8'h38) && (f_c))) begin
                        comb_bus_op = `BUS_OP_IDLE;
                        comb_pc_jr = 1;
                        comb_next = 1;
                    end
                    // condition not met: comb_next stays 0 (default), instruction ends
                    // PC is already pointing to next instruction (advanced during M0)
                end
            end
            // RET, RETI
            ...
            // RET CC
                ...
                if (m_cycle == 0) begin
                    ...
                    // else: condition not met, comb_next = 0 (default) - end instruction
                end
                else if (m_cycle == 2) begin
                    comb_ab_src = `AB_SRC_SP;
                    comb_db_src = `DB_SRC_DB;
    ...
    always @(posedge clk) begin
        ...
        else if (enable && ct_state == 2'd3) begin
