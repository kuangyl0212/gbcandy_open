...
//
// Architecture based on VerilogBoy (Wenting Zhang)
...
module gb_apu (
    ...
    // Diagnostic: NR52-style readback (for S-line debugging)
    output reg [7:0] nr52_diag,
    // Diagnostic: last write captured (for debugging lost writes)
    output reg [15:0] dbg_last_addr,
    output reg [7:0]  dbg_last_data,
    output reg        dbg_last_wr,
    output reg        dbg_seen_80,       // sticky flag: ever seen $80 written to NR52
    output reg [2:0]  dbg_seq_state,      // current frame sequencer state
    output [5:0]  dbg_ch1_max_len,     // sticky: max length_left ever seen
    output        dbg_ch1_ever_started,// sticky: trigger ever received
    output        dbg_ch1_ever_disabled, // sticky: enable ever cleared by counter
    output [5:0]  dbg_ch1_disable_len,  // length_left value when disabled
    output [7:0]  dbg_ch1_start_count,  // how many times Ch1 was triggered
    output [5:0]  dbg_ch1_len_now,       // current (non-sticky) length_left value
    output [5:0]  dbg_ch1_trigger_len,   // NRx1 value at trigger time
    output        ch1_on_flag           // Diagnostic: Ch1 enable/ON flag for NR52 bit0
);
...
always @(posedge clk) begin
    if (!resetn) begin
        ...
        if (frame_cnt == 13'd8191) begin
            ...
            dbg_seq_state <= sequencer_state + 3'b1;
        end else begin
...
wire [5:0] ch1_len_dbg, ch2_len_dbg;
...
apu_square ch1_inst (
    ...
    .level(ch1_level),
    .enable(ch1_on_flag),
    .len_debug(ch1_len_dbg),
    .dbg_max_len(dbg_ch1_max_len),
    .dbg_ever_started(dbg_ch1_ever_started),
    .dbg_ever_disabled(dbg_ch1_ever_disabled),
    .dbg_disable_len(dbg_ch1_disable_len),
    .dbg_start_count(dbg_ch1_start_count),
    .dbg_trigger_len(dbg_ch1_trigger_len)
);

assign dbg_ch1_len_now = ch1_len_dbg;
...
always @(posedge clk) begin
    if (!resetn) begin
        ...
        dbg_last_addr <= 16'h0; dbg_last_data <= 8'h0; dbg_last_wr <= 1'b0;
        dbg_seen_80 <= 1'b0;
    end else begin
        dbg_last_wr <= 1'b0;
        if (wr) begin
            ...
            if (addr_in_regs) begin
                if (addr == 16'hFF26) begin
                    ...
                    if (din[7] == 0) begin
                        ...
                        reg_nr52 <= din;
                        dbg_seen_80 <= 1'b1;   // sticky: Sound ON write detected
                    end
                end else if (sound_enable) begin
...
always @(*) begin
    ...
    if (reg_sel) begin
        if (addr_in_regs) begin
            if (addr == 16'hFF26) begin
                dout = {sound_enable, 3'b111, ch4_on_flag, ch3_on_flag, ch2_on_flag, ch1_on_flag};
            end else begin
                case (addr)
