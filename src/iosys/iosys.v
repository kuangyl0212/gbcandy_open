...
//
// Based on SNESTang iosys by nand2mario
...
//    input clkref,                   // 1/2 of mclk, for sdram access synchronization
    ...
    input rom_do_ready,             // flow control: 1 = ready to accept data

    output [31:0] mbc_info,         // MBC type/rom_mask/ram_mask from firmware
    ...
    output flash_loaded,            // Debug: firmware loaded from SPI flash
...
wire        romload_reg_ctrl_sel /* synthesis syn_keep=1 */ = mem_valid && (mem_addr == 32'h02000030);       // write 1 to start loading, 0 to finish loading
wire        romload_reg_data_sel /* synthesis syn_keep=1 */ = mem_valid && (mem_addr == 32'h02000034);       // write once to load 4 bytes
wire        mbc_info_reg_sel /* synthesis syn_keep=1 */ = mem_valid && (mem_addr == 32'h02000038);
...
assign mem_ready = ram_ready || textdisp_reg_char_sel || simpleuart_reg_div_sel || 
            romload_reg_ctrl_sel || romload_reg_data_sel || mbc_info_reg_sel || joystick_reg_sel || time_reg_sel || cycle_reg_sel || id_reg_sel ||
...
always @(posedge clk) begin
    ...
    if (romload_reg_data_sel && mem_wstrb) begin
        ...
        rom_cnt <= 2'd3;
        if (rom_do_ready) rom_do_valid <= 1;
    end
    if (rom_cnt != 2'd0 && rom_do_ready) begin      // output remaining rom_do with flow control
...
always @(posedge clk) begin
    if (~resetn) begin
        rom_loading <= 0;
    end else if (romload_reg_ctrl_sel && mem_wstrb) begin
...
reg [31:0] mbc_info_reg;
assign mbc_info = mbc_info_reg;
always @(posedge clk) begin
    if (~resetn)
        mbc_info_reg <= 0;
    else if (mbc_info_reg_sel && mem_wstrb)
        mbc_info_reg <= mem_wdata;
end
