 ...
 *
 * Based on SNESTang firmware by nand2mario
 */
...
#define OPTION_OSD_KEY_RIGHT_START 3

// SNES BSRAM is mapped at address 7MB 
volatile uint8_t *SNES_BSRAM = (volatile uint8_t *)0x07000000;

int option_osd_key = OPTION_OSD_KEY_RIGHT_START;
#define OSD_KEY_CODE (option_osd_key == OPTION_OSD_KEY_SELECT_START ? 0xC : (option_osd_key == OPTION_OSD_KEY_SELECT_RIGHT ? 0x84 : 0x88))
...
int menu_loadrom(int *choice) {
    ...
    while (1) {
        ...
        if (r == 0) {
            ...
            delay(300);
            while (1) {
                int r = joy_choice_items(TOPLINE, file_len, &active, OSD_KEY_CODE, file_names);
                if (r == 1) {
...
void menu_select_core(int verify) {
    ...
    for (;;) {
        ...
        if (joy_choice_items(2, total, &choice, OSD_KEY_CODE, file_names) == 1) {
            if (choice == 0)
                return;
            else {
                char *p;
                ...
                if (choice == 0) {
                    ...
                    if (choice == 2) {
                        if (option_osd_key == OPTION_OSD_KEY_SELECT_START)
                            ...
                            option_osd_key = OPTION_OSD_KEY_RIGHT_START;
                        else
...
int loadgbc(int rom) {
    ...
    unsigned int br;
    r = f_read(&f, load_buf, 512, &br);
    if (r != FR_OK) {
        goto loadgbc_snes_end;
    }

    uint8_t cart_type = (uint8_t)load_buf[0x147];
    uint8_t rom_size_code = (uint8_t)load_buf[0x148];
    uint8_t ram_size_code = (uint8_t)load_buf[0x149];

    uint8_t mbc_type;
    switch (cart_type) {
        case 0x00: mbc_type = 0; break;
        case 0x01: case 0x02: case 0x03: mbc_type = 1; break;
        case 0x05: case 0x06: mbc_type = 2; break;
        case 0x0F: case 0x10: case 0x11: case 0x12: case 0x13: mbc_type = 3; break;
        case 0x19: case 0x1A: case 0x1B: case 0x1C: case 0x1D: case 0x1E: mbc_type = 5; break;
        default: mbc_type = 0; break;
    }

    uint32_t rom_mask;
    switch (rom_size_code) {
        case 0: rom_mask = 0x001; break;
        case 1: rom_mask = 0x003; break;
        case 2: rom_mask = 0x007; break;
        case 3: rom_mask = 0x00F; break;
        case 4: rom_mask = 0x01F; break;
        case 5: rom_mask = 0x03F; break;
        case 6: rom_mask = 0x07F; break;
        case 7: rom_mask = 0x0FF; break;
        case 8: rom_mask = 0x1FF; break;
        default: rom_mask = 0x1FF; break;
    }

    uint32_t ram_mask;
    switch (ram_size_code) {
        case 0: ram_mask = 0x0; break;
        case 1: ram_mask = 0x0; break;
        case 2: ram_mask = 0x0; break;
        case 3: ram_mask = 0x3; break;
        case 4: ram_mask = 0xF; break;
        case 5: ram_mask = 0x7; break;
        default: ram_mask = 0x0; break;
    }

    reg_mbc_info = ((ram_mask & 0xF) << 17) | ((rom_mask & 0x1FF) << 8) | (mbc_type & 0xFF);
    DEBUG("GB: cart_type=%02x mbc=%d rom_mask=%03x ram_mask=%x\n",
          cart_type, mbc_type, rom_mask, ram_mask);
    ...
    // No dummy header for GBC - ROM data starts directly at address 0
    ...
    do {
        ...
        if ((total & 0x3fff) == 0) {
            status("");
            printf("%d/%dK", total >> 10, f_size(&f) >> 10);
        }
    } while (br == 1024);
...
int main() {
    ...
    // Debug: output before SD init
    uart_printf("GBCandy firmware starting...\n");
    uart_printf("CORE_ID=%d\n", CORE_ID);

    int sd_ret = sd_init();
    uart_printf("SD init returned: %d\n", sd_ret);
    delay(100);
    DEBUG("CORE_ID=%d\n", CORE_ID);
