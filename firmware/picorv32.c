...
// overlay_key_code: 0x84 for SELECT&RIGHT, 0xC for SELECT&START, 0x88 for RIGHT&START
int joy_choice(int start_line, int len, int *active, int overlay_key_code) {
   return joy_choice_items(start_line, len, active, overlay_key_code, 0);
}

int joy_choice_items(int start_line, int len, int *active, int overlay_key_code, char items[][256]) {
   ...
   if (items) {
      ...
      for (int i = 0; i < len; i++) {
         ...
         if (!s) continue;
         int slen = strlen(s);
         if (slen <= vis_width) continue;
         cursor(1, start_line + i);
         if (i == *active) {
            static int scroll_pos = 0;
            static int scroll_last = -1;
            static uint32_t scroll_time = 0;
            if (scroll_last != i) {
               scroll_pos = 0;
               scroll_last = i;
               scroll_time = time_millis();
            }
            if (time_millis() - scroll_time > 400) {
               scroll_pos++;
               if (scroll_pos > slen - vis_width)
                  scroll_pos = 0;
               scroll_time = time_millis();
            }
            for (int j = 0; j < vis_width; j++)
               putchar(s[scroll_pos + j]);
         } else {
            for (int j = 0; j < vis_width; j++)
               putchar(s[j]);
         }
